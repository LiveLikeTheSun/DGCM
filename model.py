...
class Model(object):
    ...
    def evaluation(self, length, clicks, probablity):
        perplexity_at_rank = torch.tensor([0.0] * 10)  # 6245930
        tmp_ll = torch.tensor([0.0] * 10)
        batchs = int(length/self.args.test_batch_size)
        for m in range(batchs):
            for i in range(self.args.test_batch_size):
                probs = []
                for x in range(i+m*self.args.test_batch_size*10, len(clicks) * 10, self.args.test_batch_size):
                    if len(probs) < 10:
                        probs.append(probablity[x])
                    else:
                        break
                one_perplexity = self.compute_perplexity(probs, clicks[i+m*self.args.test_batch_size])
                perplexity_at_rank = perplexity_at_rank + one_perplexity
                tmp = self.compute_ll(probs, clicks[i+m*self.args.test_batch_size])
                tmp_ll = tmp_ll + tmp
        loglikelihood = tmp_ll.sum() / (len(clicks) * 10)
        perplexity = torch.tensor(0.0)
        for i in range(len(perplexity_at_rank)):
            perplexity_at_rank[i] = 2 ** ((- perplexity_at_rank[i]) / len(clicks))
            perplexity += perplexity_at_rank[i]
        perplexity = perplexity / 10

        print('loglikelihood:', loglikelihood)
        print('perplexity:', perplexity)
        print('perplexity_at_rank:', perplexity_at_rank)
    ...
    def get_query_output(self, length, output):
        batchs = int(length/self.args.test_batch_size)
        query_output = []
        for m in range(batchs):
            for i in range(self.args.test_batch_size):
                tmp = []
                for x in range(i+m*self.args.test_batch_size*10, length * 10, self.args.test_batch_size):
                    if len(tmp) < 10:
                        tmp.append(output[x])
                query_output.append(tmp)
        return query_output
    ...
    def train(self, epoch, data, test_data):
        ...
        for step, batch in enumerate(train_loader):
            ...
            for se in sessions: # 得到一个batch的feature和邻接矩阵
                tmp_title_id = []
                sess_feas = np.zeros((len(self.all_sess_img_id[se])+self.args.num_fix_node, self.args.input_feature_size)).tolist()
                show_idx = self.all_sess_show_idx[se]
                for i in show_idx:
                    sess_feas[self.args.num_start_idx_sni+show_idx.index(i)][:self.args.emb_size] = self.query_sni2fea[querys[num]][i]
                for i in self.all_sess_img_id[se]:
                    sess_feas[self.args.num_fix_node+self.all_sess_img_id[se].index(i)][:self.args.emb_size] = self.img_fea[i]
                train_fea += sess_feas
                for i in show_idx:
    ...
    def test(self, idx_test):
        ...
        test_loader = DataLoader(idx_test, batch_size=self.args.test_batch_size, shuffle=False, drop_last=True)
        true_clicks = []
        probablity = []
        hidden = []
        ...
        for step, batch in enumerate(test_loader):
            ...
            url_id = []
            results_id = []
            for j in idx_test[step*self.args.test_batch_size:(step+1)*self.args.test_batch_size]:
                sessions.append(j['session'])
                querys.append(j['query'])
                results_id.append(j['result_id'])
                clicks.append(j['click'])
                tmp_true_clicks.append(j['click'][1:])
                url_id.append(j['url_ids'])
            true_clicks += tmp_true_clicks
            ...
            for se in sessions: # 得到一个batch的feature和邻接矩阵
                tmp_title_id = []
                sess_feas = np.zeros((len(self.all_sess_img_id[se])+self.args.num_fix_node, self.args.emb_size+56)).tolist()
                show_idx = self.all_sess_show_idx[se]
                for i in show_idx:
                    sess_feas[self.args.num_start_idx_sni+show_idx.index(i)][:self.args.emb_size] = self.query_sni2fea[querys[num]][i]
                for i in self.all_sess_img_id[se]:
                    ...
                    sess_feas[self.args.num_fix_node+self.all_sess_img_id[se].index(i)][:self.args.emb_size] = self.img_fea[i]
                    # except:
                    #     sess_feas[31+self.all_sess_img_id[se].index(i)][:self.args.emb_size] = self.img_fea_160[i].tolist() + [random.uniform(0.0, 1.0) for _ in range(self.args.emb_size-160)]
                test_fea += sess_feas
                for i in show_idx:
                    tmp_title_id.append(self.query_title2id[querys[num]][i])
                title_id.append(tmp_title_id)
            ...
            for i in range(10):
                ...
                for cli in tmp_true_clicks:
                    result_click.append([cli[i]])
                
                for docs in doc_id:
                    result_id.append(docs[i])
                # if self.args.cuda:
                #     test_adj = torch.tensor(test_adj).cuda()
                #     doc_id = torch.tensor(doc_id).cuda()
                #     result_clicks = torch.tensor(result_clicks).cuda()
                #     querys = torch.tensor(querys).cuda()
                #     url_id = torch.tensor(url_id).cuda()
                #     title_id = torch.tensor(title_id).cuda()
                
               
                hidden_state, output, x_part = self.model(i, x, torch.tensor(test_adj).cuda(), torch.tensor(doc_id).cuda(), torch.tensor(result_clicks).cuda(), torch.tensor(querys).cuda(), torch.tensor(url_id).cuda(), torch.tensor(title_id).cuda())  # , False
            
                # hidden_state, output, x_part = self.model(i, x, test_adj, doc_id, result_clicks, querys, url_id, title_id)  # , False
                output = output.detach().cpu()
                probablity += output[result_id]
                
                loss += criterion(output[result_id].cuda(), torch.tensor(result_click).float().cuda())
                x = torch.cat((x_part, hidden_state), dim=1)
                hidden_part = hidden_state[result_id].detach().cpu()
                hidden += hidden_part.tolist()
                
        print('test_loss:', loss.item() / (int(len(idx_test)/self.args.test_batch_size)*10))
